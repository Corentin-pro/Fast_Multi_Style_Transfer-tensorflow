    ...
    def test(self, Train_flag=True):
        for fn in os.listdir(self.test_dataset):
            ...
            if Train_flag:
                ...
                Image.fromarray(im_output.astype(np.uint8)).save(os.path.join(train_output_dir, filename))
            else:
                test_output_dir = os.path.join(self.project_dir, 'test_result')
                filename = fn[:-4] + '_' + str(style_idx) + '_output.bmp'
                Image.fromarray(im_output.astype(np.uint8)).save(os.path.join(test_output_dir, filename))
            print(filename)
